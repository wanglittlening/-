...
<div id="chat-control">
  <select id="model-select">
    ...
    <option value="net-gpt-3.5-turbo">net-gpt-3.5-turbo</option>
...
function getAnyOnekey() {
  const randomIndex = Math.floor(Math.random() * keys.length);
  return keys[randomIndex];
}
headers.append("Authorization", `Bearer ${getAnyOnekey()}`);
...
function chat() {
  ...
  fetch("https://free.v36.cm/v1/chat/completions", requestOptions)
    .then(async response => {
...
function appendToConversationHistory(role, content, messageElementId) {
  ...
  // 将普通文本中的换行符替换为 <br> 标签，确保换行正常显示
  formattedContent = formattedContent.replace(/\n/g, "<br />");

  // 检查内容中是否包含代码块标记
  const codeBlockRegex = /```(\w+)?([\s\S]*?)```|`([^`]+)`/g;
  let formattedContent = content.replace(codeBlockRegex, (match, lang, blockContent, inlineContent) => {
    if (blockContent) {
      // 处理多行代码块，将HTML标签字符转义
      blockContent = escapeHtml(blockContent).trim(); // 去除代码块前后空行
      lang = lang || "plaintext"; // 如果没有指定语言，默认为 "plaintext"
      return `<pre><span class="code-type">${lang}</span><code>${blockContent.replace(/<\s*br\s*\/?\s*>/g,"\n")}</code></pre>`;
    } else if (inlineContent) {
      // 处理行内代码，将HTML标签字符转义
      inlineContent = escapeHtml(inlineContent);
      return `<code>${inlineContent}</code>`;
    }
    return match;
  });
