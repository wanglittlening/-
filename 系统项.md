...
function appendToConversationHistory(role, content, messageElementId) {
  ...
  let formattedContent = content.replace(codeBlockRegex, (match, blockType, blockContent, inlineContent) => {
    if (blockType && blockContent) {
      // 处理带类型的代码块，如 ```html ... ```，将HTML标签字符转义
      blockContent = escapeHtml(blockContent);
      return `<pre><code class="code-block"><span class="code-type">${blockType}</span>${blockContent}</code></pre>`;
