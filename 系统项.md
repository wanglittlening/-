...
function appendToConversationHistory(role, content, messageElementId) {
  ...
  let formattedContent = content.replace(codeBlockRegex, (match, language, blockContent, inlineContent) => {
    if (blockContent) {
      // 处理多行代码块，将HTML标签字符转义，并添加代码类型
      blockContent = escapeHtml(blockContent);
      return `
        <div>
          <code class="type">${language || 'plaintext'}</code>
          <pre><code>${blockContent}</code></pre>
        </div>
      `;
