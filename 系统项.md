...
<div id="prompt">
  ...
  <span key="[RP]" class="optional">📄读当前页页</span>
  <span key="[SD]" class="optional" style="display:none;">🗂️基于搜索项</span>
...
const providerList = [
  ...
  ""
]
// 选择一个服务提供者信息
let sessionProvider = {
...
const getInitConversationHistory = (()=>{
  ...
  return async function(userInput) {
    ...
    if("[WS]" === prompt) {
      ...
      if(bodyHtml.length > (sessionProviderSpecific?.contextLimit || 4000 ) {
        bodyHtml = await window.MS_SCRIPT_ENV.getSelectedText("由于页面过大，请手动选择文本")
      }
      prompt = `
      ---------------
      下面是用户在页面html或用户选择的文本信息，请你结合下面页面数据回复用户问题。
      ---------------
      ${bodyHtml}
      ---------------
      `;
    }else if("[SD]" === prompt && isAlreadySwitched) {
      const searchData = (window.MS_SCRIPT_ENV.getSearchDB() ?? []).map(e => `${(e.title ?? '').replace(/\[.*\]/gm,'').trim()}是${e.desc.includes('-无描述-')?'':e.desc}`);
      prompt = `
        ---------------
