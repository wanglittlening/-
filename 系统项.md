...
function appendToConversationHistory(role, content, messageElementId) { 
  ...
  // 判断 content 是否包含代码块的标识
  let formattedContent = content;

  // 如果内容看起来像代码块（例如，包含多个行并且以特定标识开头）
  if (content.includes('```')) {
    // 假设代码块用 ``` 包裹，处理成 <pre><code> 形式
    formattedContent = content.replace(/```(.*?)```/gs, "<pre><code>$1</code></pre>");
  } else {
    // 将换行符替换为 <br> 标签
    formattedContent = content.replace(/\n/g, "<br>");
  }

  // 设置 messageElement 的 innerHTML 来处理 HTML 内容
