...
<div id="prompt">
  ...
  <span key="[RP]" class="optional" title="如果页面内容很多（超过3000字符），需要选中文本再提问！">📄选中的文本（TODO）</span>
  <span key="[SD]" class="optional">🗂️基于搜索项（TODO）</span>
...
const getInitConversationHistory = (()=>{
  ...
  return async function(userInput) {
    ...
    if("[WS]" === prompt) {
      ...
      prompt = `
      ---------------
      下面是当前页面html或页面的文本选中的文本数据，请你结合下面页面数据回复用户问题。
      ---------------
      ${bodyHtml}
      ---------------
      `;
