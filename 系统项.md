...
<div id="prompt">
  ...
  <span key="[RP]" class="optional">📄读当前页面</span>
  <span key="[SD]" class="optional">🗂️基于搜索项</span>
  <input placeholder="📝定义AI角色..回车添" />
</div>
<div id="chat-control">
  <select id="model-select">
  </select>
  <input type="text" id="role_add_input">
...
document.querySelector('#chat-control #role_add_input').addEventListener('keydown', function(event) {
  // 判断是否是回车键
  if (event.key === 'Enter') {
    ...
    if (inputValue) {
      ...
      // 将新的 span 添加到 #prompt div 中
      document.getElementById('prompt').appendChild(newSpan);
...
// 添加点击事件监听器
document.querySelectorAll('#prompt > span').forEach(function(span) {
  span.addEventListener('click', function() {
    ...
    if (this.classList.contains('selected')) {
      ...
      document.querySelectorAll('#prompt > span').forEach(function(otherSpan) {
...
function getSelectedKeyValue() {
  ...
  return selectedSpan ? selectedSpan.getAttribute('key') : null;
}
...
const getInitConversationHistory = (()=>{
  ...
  return async function(userInput) {
    let prompt = getSelectedKeyValue() ?? undefined;
    ...
    if("[WS]" === prompt) {
      ...
      prompt = `
      ---------------
      下面是当前页面html数据，请你结合下面页面数据回复用户问题。
      ---------------
