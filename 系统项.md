...
headers.append("Authorization", `Bearer sk-4IIpTOt80NdtI6G912Da1fF87f3a4a0bBb062eA4477cC252`);
...
function appendToConversationHistory(role, content, messageElementId) {
  ...
  let formattedContent = content.replace(codeBlockRegex, (match, lang, blockContent, inlineContent) => {
    if (blockContent) {
      // 处理多行代码块，将HTML标签字符转义
      blockContent = escapeHtml(blockContent).trim();  // 去除前后空行
      lang = lang || "plaintext"; // 如果没有指定语言，默认为 "plaintext"
      return `<pre><span class="code-type">${lang}</span><code>${blockContent}</code></pre>`;
    } else if (inlineContent) {
      // 使用 innerText 插入行内代码，并保留反引号
      const escapedInlineContent = escapeHtml(inlineContent);
      return `<span class="inline-code">\`${escapedInlineContent}\`</span>`;
    }
    return match;
  });
