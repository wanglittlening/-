...
<div id="prompt">
  ...
  <input id="role_add_input" placeholder="📝定义AI角色..回车添" />
</div>
<div id="chat-control">
...
document.querySelector('#prompt #role_add_input').addEventListener('keydown', function(event) {
  ...
  if (event.key === 'Enter') {
    ...
    if (inputValue) {
      ...
      newSpan.textContent = ivArr[0];
      newSpan.setAttribute('key', ivArr.length > 1 ? ivArr[1] : ivArr[0]);

      // 将新的 span 添加到 #prompt div 中
      document.querySelector('#prompt #prompt-list').appendChild(newSpan);
...
// 角色/知识库点击
// 绑定事件监听器到 #prompt-list 父元素
document.querySelector('#prompt-list').addEventListener('click', function(event) {
  // 确保点击的元素是 span
